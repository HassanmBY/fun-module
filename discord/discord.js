...
simulatedMessages.forEach(m => {
	setTimeout(() => {
		sendMessage(m);
		// Show Discord notification with avatar (wait for functions to be available)
		if (typeof window.showDiscordNotification === "function") {
			const avatarPath = `./discord/${getRandomAvatar()}`;
			window.showDiscordNotification(m.title, m.text, m.channelid, avatarPath);
		} else if (typeof showDiscordNotification === "function") {
			const avatarPath = `./discord/${getRandomAvatar()}`;
			showDiscordNotification(m.title, m.text, m.channelid, avatarPath);
		}
		// Show mention notification (only for some messages)
		if (
			(typeof window.showDiscordMentionNotification === "function" ||
				typeof showDiscordMentionNotification === "function") &&
			Math.random() > 0.7
		) {
			const func =
				window.showDiscordMentionNotification || showDiscordMentionNotification;
			func(m.title, m.text, m.channelid);
		}
		// Show notification with sound (only for some messages)
		if (
			(typeof window.showNotificationWithSound === "function" ||
				typeof showNotificationWithSound === "function") &&
			Math.random() > 0.5
		) {
			const func = window.showNotificationWithSound || showNotificationWithSound;
			func(m.title, m.text);
		}
